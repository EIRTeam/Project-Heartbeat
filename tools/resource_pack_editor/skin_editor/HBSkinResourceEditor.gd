...
onready var font_name_text_input = preload("res://menus/HBTextInput.tscn").instance()
onready var texture_name_text_input := preload("res://menus/HBTextInput.tscn").instance()
...
func set_texture_import_mode(val):
	...
	match texture_import_mode:
		TEXTURE_IMPORT_MODE.NORMAL:
			...
			open_texture_file_dialog.mode = FileDialog.MODE_OPEN_FILE
...
func _ready():
	...
	delete_resource_confirmation_dialog.connect("confirmed", self, "_on_delete_resource_confirmed")
	...
	open_font_file_dialog.mode = FileDialog.MODE_OPEN_FILE
	open_texture_file_dialog.mode = FileDialog.MODE_OPEN_FILE
	open_texture_file_dialog.connect("file_selected", self, "user_add_texture_from_path")
	open_texture_file_dialog.connect("files_selected", self, "user_add_animated_texture_from_paths")
	open_font_file_dialog.connect("file_selected", self, "user_add_font_from_path")
	...
	font_name_text_input.connect("entered", self, "_on_font_name_text_entered")
	texture_name_text_input.connect("entered", self, "_on_texture_name_text_entered")
	...
	add_animated_texture_button.connect("pressed", self, "set_texture_import_mode", [TEXTURE_IMPORT_MODE.ANIMATED])
	...
	tree.connect("button_pressed", self, "_on_item_button_pressed")

func _on_font_name_text_entered(font_name: String):
	font_name_text_input.hide()
	if resource_storage.has_font(font_name):
...
func user_add_texture_from_path(path: String):
	...
	if dir.file_exists(path):
		...
		if not dir.dir_exists(skin_resources_path):
			dir.make_dir_recursive(skin_resources_path)
		var f := File.new()
		if f.open(path, File.READ) == OK:
			var array := f.get_buffer(4)
			if array == PoolByteArray([0x89, 0x50, 0x4E, 0x47]):
				f.close()
				var new_path := skin_resources_path.plus_file(HBUtils.get_valid_filename(currently_creating_texture_name) + ".png")
...
func user_add_font_from_path(path: String):
	...
	if dir.file_exists(path):
		...
		if f.open(path, File.READ) == OK:
			var array := f.get_buffer(4)
			var extension := ".otf"
			var is_otf = array == PoolByteArray([0x4F, 0x54, 0x54, 0x4F])
			var is_ttf = array == PoolByteArray([0x00, 0x01, 0x00, 0x00])
			if is_otf or is_ttf:
				f.close()
				if is_ttf:
					extension = ".ttf"
				var new_path := skin_resources_path.plus_file(HBUtils.get_valid_filename(currently_creating_font_name) + extension)
				if dir.file_exists(new_path):
					show_error("File with name %s already exists in skin" % new_path.get_file())
				else:
					dir.copy(path, new_path)
...
func add_animated_texture(texture_name: String, texture: AnimatedTexture):
	...
	var item := tree.create_item()
	item.set_text(0, texture_name)
	item.set_text(1, "Animated Texture")
...
func delete_resource(item: TreeItem):
	...
	match resource_type:
		RESOURCE_TYPE_FONT:
			var dir := Directory.new()
			var font := resource_storage.get_font(item.get_meta("font_name")) as DynamicFontData
			dir.remove(font.font_path)
		...
		RESOURCE_TYPE_TEXTURE:
			...
			if item.get_meta("is_animated_texture"):
				...
				for frame in range(animated_texture.frames):
					dir.remove(animated_texture.get_frame_texture(frame).get_meta("animated_texture_path"))
			else:
...
func add_font(font_name: String, path: String, font: DynamicFontData):
			...
			RESOURCE_TYPE_TEXTURE:
				...
				if is_animated:
					...
					resources.textures[texture_name] = {
						"fps": animated_texture.fps,
						"frames": frames
					}
					for i in range(animated_texture.frames):
						frames.push_back({
							"delay": animated_texture.get_frame_delay(i),
