...
func set_entry(_entry: YoutubeDL.CachingQueueEntry):
	if entry:
		entry.disconnect("download_progress_changed", self, "_on_download_progress_changed")
	entry = _entry
	entry.connect("download_progress_changed", self, "_on_download_progress_changed", [], CONNECT_DEFERRED)
	var song = entry.song
