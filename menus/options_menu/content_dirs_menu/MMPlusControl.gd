...
func update_error():
	...
	if HBGame.mmplus_error == HBGame.MMPLUS_ERROR.NEEDS_RESTART:
		...
		mm_error_label.append_bbcode(tr("A restart is needed for changes to apply"))
		mm_error_label.pop()
	...
	if HBGame.mmplus_loader:
		if HBGame.mmplus_loader.has_fatal_error():
			...
			mm_error_label.push_underline()
			mm_error_label.push_color(Color.lightcoral)
			mm_error_label.append_bbcode(tr("%d fatal errors were found:" % HBGame.mmplus_loader.fatal_error_messages.size()))
			mm_error_label.pop()
			mm_error_label.pop()
			mm_error_label.newline()
			for error in HBGame.mmplus_loader.fatal_error_messages:
				mm_error_label.append_bbcode(error)
				mm_error_label.newline()
		if HBGame.mmplus_loader.has_error():
			mm_error_label.newline()
			mm_error_label.push_underline()
			mm_error_label.push_color(Color.lightgoldenrod)
			mm_error_label.append_bbcode(tr("%d non-fatal errors were found:" % HBGame.mmplus_loader.error_messages.size()))
			mm_error_label.pop()
			mm_error_label.pop()
			mm_error_label.newline()
