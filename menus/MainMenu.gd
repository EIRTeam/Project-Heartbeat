...
func _ready():
	...
	MENUS["results"].left.connect("select_song", MENUS["song_list_preview"].right, "select_song")
	
	MENUS["results_mp"].left.connect("show_song_results_mp", MENUS["mp_leaderboard"].right.get_leadearboard_control(), "set_entries")
