...
func create_controller_map(joypad):
	...
	for i in range(0, 4, 2):
		var visualizer = ANALOG_GAMEPAD_VISUALIZER.instance()
		visualizer.axis_x = i
	...
	vbox_container.alignment = VBoxContainer.ALIGN_CENTER
	
	var slider1 = get_slider_axis(JOY_ANALOG_L2, joypad)
	vbox_container.add_child(slider1)
	
	var slider2 = get_slider_axis(JOY_ANALOG_R2, joypad)
	...
	button_control.rect_min_size = Vector2(200, 0)
